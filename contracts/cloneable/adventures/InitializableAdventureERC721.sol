...
import "./InitializableAdventureWhitelist.sol";
import "../token/ERC721/InitializableERC721.sol";
import "../../adventures/IAdventurous.sol";
 ...
 */
abstract contract InitializableAdventureERC721 is InitializableERC721, InitializableAdventureWhitelist, IAdventurous {

    error InitializableAdventureERC721__AdventureApprovalToCaller();
    error InitializableAdventureERC721__AlreadyInitializedAdventureERC721();
    error InitializableAdventureERC721__AlreadyOnQuest();
    error InitializableAdventureERC721__AnActiveQuestIsPreventingTransfers();
    error InitializableAdventureERC721__CallerNotApprovedForAdventure();
    error InitializableAdventureERC721__CallerNotTokenOwner();
    error InitializableAdventureERC721__MaxSimultaneousQuestsCannotBeZero();
    error InitializableAdventureERC721__MaxSimultaneousQuestsExceeded();
    error InitializableAdventureERC721__NotOnQuest();
    error InitializableAdventureERC721__QuestIdOutOfRange();
    error InitializableAdventureERC721__TooManyActiveQuests();
    ...
    function initializeAdventureERC721(uint256 maxSimultaneousQuests_) public onlyOwner {
        if(initializedAdventureERC721) {
            revert InitializableAdventureERC721__AlreadyInitializedAdventureERC721();
        }
    ...
    function setAdventuresApprovedForAll(address operator, bool approved) external {
        ...
        if(tokenOwner == operator) {
            revert InitializableAdventureERC721__AdventureApprovalToCaller();
        }
    ...
    function isParticipatingInQuest(uint256 tokenId, address adventure, uint256 questId) public override view returns (bool participatingInQuest, uint256 startTimestamp, uint256 index) {
        if(questId > type(uint32).max) {
            revert InitializableAdventureERC721__QuestIdOutOfRange();
        }
    ...
    function _enterQuest(uint256 tokenId, address adventure, uint256 questId) internal {
        ...
        if(participatingInQuest) {
            revert InitializableAdventureERC721__AlreadyOnQuest();
        }

        uint256 currentQuestCount = getQuestCount(tokenId, adventure);
        if(currentQuestCount >= _maxSimultaneousQuests) {
            revert InitializableAdventureERC721__TooManyActiveQuests();
        }
    ...
    function _exitQuest(uint256 tokenId, address adventure, uint256 questId) internal {
        ...
        if(!participatingInQuest) {
            revert InitializableAdventureERC721__NotOnQuest();
        }
    ...
    function _requireCallerApprovedForAdventure(uint256 tokenId) internal view {
        if(!areAdventuresApprovedForAll(ownerOf(tokenId), _msgSender())) {
            revert InitializableAdventureERC721__CallerNotApprovedForAdventure();
        }
    }
    ...
    function _requireCallerOwnsToken(uint256 tokenId) internal view {
        if(ownerOf(tokenId) != _msgSender()) {
            revert InitializableAdventureERC721__CallerNotTokenOwner();
        }
    }
    ...
    function _validateMaxSimultaneousQuests(uint256 maxSimultaneousQuests_) internal pure {
        if(maxSimultaneousQuests_ == 0) {
            revert InitializableAdventureERC721__MaxSimultaneousQuestsCannotBeZero();
        }

        if(maxSimultaneousQuests_ > MAX_CONCURRENT_QUESTS) {
            revert InitializableAdventureERC721__MaxSimultaneousQuestsExceeded();
        }
    }
}
