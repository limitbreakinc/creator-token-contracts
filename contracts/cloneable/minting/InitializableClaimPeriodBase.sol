 ...
 *
 */
abstract contract InitializableClaimPeriodBase is InitializableOwnable {

    error InitializableClaimPeriodBase__ClaimsMustBeClosedToReopen();
    error InitializableClaimPeriodBase__ClaimPeriodIsNotOpen();
    error InitializableClaimPeriodBase__ClaimPeriodMustBeClosedInTheFuture();
    ...
    function openClaims(uint256 claimPeriodClosingTimestamp_) external onlyOwner {
        if(claimPeriodClosingTimestamp_ <= block.timestamp) {
            revert InitializableClaimPeriodBase__ClaimPeriodMustBeClosedInTheFuture();
        }
        ...
        if(claimPeriodInitialized) {
            if(block.timestamp < claimPeriodClosingTimestamp) {
                revert InitializableClaimPeriodBase__ClaimsMustBeClosedToReopen();
            }
        } else {
    ...
    function _requireClaimsOpen() internal view {
        if(!_isClaimPeriodOpen()) {
            revert InitializableClaimPeriodBase__ClaimPeriodIsNotOpen();
        }
    }
